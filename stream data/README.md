...
특정 위치부터 데이터를 가져오려면 `config.py` 파일에 `BQ_OFFSET` 변수를 추가하고, `queries.py` 파일에 ORDER BY와 OFFSET 구문을 추가하세요:
...
3. 쿼리 마지막에 ORDER BY와 OFFSET 구문 추가:
```python
WHERE _TABLE_SUFFIX = '{BQ_DATE_SUFFIX}'
ORDER BY t.visitStartTime  # 중요: BigQuery에서 OFFSET을 사용하려면 ORDER BY가 필요합니다
OFFSET {BQ_OFFSET}
LIMIT {BQ_LIMIT}
```

> **주의**: BigQuery에서 OFFSET을 사용할 때는 반드시 ORDER BY 구문이 선행되어야 합니다. ORDER BY 없이 OFFSET을 사용하면 "Syntax error: Expected end of input but got keyword OFFSET" 오류가 발생합니다.
